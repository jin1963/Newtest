...
async function connectWallet() {
  if (window.ethereum) {
    ...
    try {
      ...
      // ตรวจสอบว่าอยู่บน BNB Chain (Chain ID = 56)
      if (chainId !== 56) {
        try {
          ...
          window.location.reload(); // reload เพื่อให้แสดงสถานะหลังสลับ chain
        } catch (switchError) {
      ...
      alert("❌ การเชื่อมต่อกระเป๋าล้มเหลว: " + e.message);
    }
  } else {
...
async function purchase() {
  ...
  const usdtAmount = web3.utils.toWei(amount, "mwei"); // USDT มี 6 decimals
  ...
  try {
    ...
    document.getElementById("status").innerText = "✅ ซื้อและ Stake KJC สำเร็จ";
  } catch (e) {
    document.getElementById("status").innerText =
